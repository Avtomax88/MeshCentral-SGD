...
//
// Этот файл MeshCentral использует ДВОЯКО:
//  1) на сервере (Node.js) через require(...)[shortName] — там нет ни
//     window, ни document;
//  2) в браузере, где вытаскивает и заново выполняет КАЖДУЮ функцию из
//     obj.exports отдельно от остального файла, без доступа к соседним
//     функциям/переменным — единственное надёжное общее место оттуда и
//     отсюда — это window.
//
// Поэтому весь браузерный код целиком обёрнут проверкой typeof window,
// а вспомогательные функции — явные свойства window, а не просто "var"/
// "function" на верхнем уровне файла.

if (typeof window !== 'undefined') {
	// Правь эту константу под свой адрес дашборда:
	window.SG_DASHBOARD_URL = 'https://sgd.supporthound.ru';

	// Диагностика: если этой строки нет в консоли браузера — значит сам
	// файл плагина до браузера не долетает вообще.
